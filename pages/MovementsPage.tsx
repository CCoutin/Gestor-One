...
const pageConfig = {
    entrada: {
        ...
        headers: ['Material', 'Quantidade', 'Valor Total', 'Colaborador', 'Data', 'Nota Fiscal']
    },
    saida: {
        title: 'Registro de Saídas',
        buttonText: 'Nova Saída',
        modalTitle: 'Registrar Nova Saída',
        headers: ['Material', 'Quantidade', 'Valor Total', 'Colaborador', 'Data']
    },
    consumo: {
        title: 'Registro de Consumo',
        buttonText: 'Novo Consumo',
        modalTitle: 'Registrar Novo Consumo',
        headers: ['Material', 'Quantidade', 'Custo Total', 'Colaborador', 'Data']
    }
}
...
const MovementsPage: React.FC<MovementsPageProps> = ({ type }) => {
  ...
  const canAdd = user?.role === UserRole.OPERADOR || user?.role === UserRole.GERENTE || user?.role === UserRole.DIRETOR || user?.role === UserRole.VISITANTE;
  const canEdit = user?.role === UserRole.OPERADOR || user?.role === UserRole.GERENTE || user?.role === UserRole.DIRETOR || user?.role === UserRole.VISITANTE;
            ...
            <Card>
                ...
                {movements.map(mov => (
                    ...
                    <td className="p-4 font-semibold text-slate-700">
                        {formatCurrency(getMovementValue(mov))}
                    </td>
