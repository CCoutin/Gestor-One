...
const AccessManagementPage: React.FC = () => {
    ...
    const roleColorMap: { [key in UserRole]: string } = {
        ...
        [UserRole.VISITANTE]: 'bg-green-100 text-green-800',
    };
    ...
    return (
        <div className="space-y-6">
            ...
            {loggedInUser?.role === UserRole.VISITANTE && (
                <div className="p-4 bg-green-100 border-l-4 border-green-500 text-green-800 rounded-r-md">
                    <p className="font-bold">Modo Visitante</p>
                    <p className="text-sm">Você tem permissão total para testar as funcionalidades de gerenciamento de acesso.</p>
                </div>
            )}
        </div>
    );
