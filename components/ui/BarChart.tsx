...
interface BarChartProps {
  data: ChartData[];
  height?: number;
  format?: 'currency' | 'number' | 'days'; // Adicionado prop de formato
}

const BarChart: React.FC<BarChartProps> = ({ data, height = 300, format = 'currency' }) => {
  const maxValue = Math.max(...data.map(d => d.value), 0);
  const chartPadding = { top: 20, right: 20, bottom: 30, left: 50 };
  const chartWidth = 500; // Fixed width for SVG viewport
  const chartHeight = height;

  const barWidth = (chartWidth - chartPadding.left - chartPadding.right) / Math.max(data.length, 1) * 0.8;
  const barSpacing = (chartWidth - chartPadding.left - chartPadding.right) / Math.max(data.length, 1) * 0.2;

  const yScale = (value: number) => {
    if (maxValue === 0) return chartHeight - chartPadding.bottom;
    ...
    <div className="w-full overflow-x-auto">
        <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="min-w-[500px]" aria-labelledby="chart-title" role="img">
                ...
                return(
                    <g key={i}>
                        <text x={chartPadding.left - 8} y={y} textAnchor="end" alignmentBaseline="middle" className="text-xs fill-slate-500">
                           {formatValue(value).replace('R$', '').replace('dias', '').trim()}
                        </text>
            ...
            {data.map((item, index) => {
                ...
                const displayHeight = barHeight > 0 ? barHeight : 0;
                ...
                return (
                    <g key={item.label} className="group">
                        <rect
                            ...
                            className={`${format === 'days' ? 'fill-red-400 hover:fill-red-500' : 'fill-blue-500 hover:fill-blue-600'} transition-colors`}
                        />
                        <text x={x + barWidth / 2} y={chartHeight - chartPadding.bottom + 15} textAnchor="middle" className="text-xs fill-slate-600 font-medium truncate" style={{maxWidth: barWidth}}>
            ...
            {data.length === 0 && (
                <text x={chartWidth/2} y={chartHeight/2} textAnchor="middle" className="fill-slate-400">
